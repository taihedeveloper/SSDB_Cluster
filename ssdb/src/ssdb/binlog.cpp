...
int64_t Binlog::ttl() const {
	return *reinterpret_cast<const int64_t*>(buf.data() + buf.size() - sizeof(int64_t));
}

const Bytes Binlog::key() const{
	return Bytes(buf.data() + HEADER_LEN, buf.size() - HEADER_LEN - sizeof(int64_t));
}
...
const int64_t SYNC_LOG_QUEUE_SLOT = -1LL;

/* type(1)|uint64|filler(int64) */
...
static inline std::string encode_seq_key(uint64_t seq){
    ...
    ret.append((char *)&SYNC_LOG_QUEUE_SLOT, sizeof(int64_t));
	return ret;
}

static inline uint64_t decode_seq_key(const leveldb::Slice &key){
	uint64_t seq = 0;
	if(key.size() == (sizeof(uint64_t) * 2 + 1) && key.data()[0] == DataType::SYNCLOG){
...
int BinlogQueue::find_last(Binlog *log, const leveldb::Snapshot *snapshot) const{
	uint64_t ret = 0;
