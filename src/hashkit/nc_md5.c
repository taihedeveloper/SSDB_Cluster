...
MD5_Update(MD5_CTX *ctx, void *data, unsigned long size)
{
    ...
    if (used) {
        ...
        memcpy(&ctx->buffer[used], data, md5_free);
        data = (unsigned char *)data + md5_free;
        size -= md5_free;
...
MD5_Final(unsigned char *result, MD5_CTX *ctx)
{
    ...
    if (md5_free < 8) {
        memset(&ctx->buffer[used], 0, md5_free);
        body(ctx, ctx->buffer, 64);
        used = 0;
        md5_free = 64;
    }

    memset(&ctx->buffer[used], 0, md5_free - 8);
