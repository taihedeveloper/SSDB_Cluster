...
Status ClientImpl::slot_premigrating(const std::string &slot){
	const std::vector<std::string> *resp;
	resp = this->request("slot_premigrating", slot);
	Status s(resp);
	return s;
}

Status ClientImpl::migrate_slot(const std::string &slot, const std::string &ip,
		int64_t port, int64_t timeout, int64_t speed){
	const std::vector<std::string> *resp;
	resp = this->request("migrate_slot", slot, ip, str(port), str(timeout), str(speed));
	Status s(resp);
	return s;
}
Status ClientImpl::slot_postmigrating(const std::string &slot){
	const std::vector<std::string> *resp;
	resp = this->request("slot_postmigrating", slot);
	Status s(resp);
	return s;
}

Status ClientImpl::slot_preimporting(const std::string &slot){
	const std::vector<std::string> *resp;
	resp = this->request("slot_preimporting", slot);
	Status s(resp);
	return s;
}

Status ClientImpl::slot_postimporting(const std::string &slot){
	const std::vector<std::string> *resp;
